...
public abstract class ModulePlacement {
	...
	private Map<Integer, Map<String, List<Integer>>> moduleInstanceListMap;
...
//			System.out.println("Creating "+module.getName()+" on device "+device.getName());
	...
	public Map<Integer, Map<String, List<Integer>>> getModuleInstanceListMap() {
		return moduleInstanceListMap;
	}

	public void setModuleInstanceListMap(Map<Integer, Map<String, List<Integer>>> moduleInstanceListMap) {
		this.moduleInstanceListMap = moduleInstanceListMap;
	}

	public Map<Integer, Map<String, List<Integer>>> getModuleDownstreamInstances() {
		if (this.getModuleInstanceListMap() == null) {
			return new HashMap<>();
		}

		// gets the cloud device
		FogDevice cloud = getDeviceByName("cloud");
		Map<Integer, Map<String, List<Integer>>> downstreamInstanceCountMap = new HashMap<>();
		for (FogDevice device: getFogDevices()) {
			downstreamInstanceCountMap.put(device.getId(), new HashMap<>());
		}
		getModuleDownstreamInstances(cloud.getId(),
									 downstreamInstanceCountMap);
		return downstreamInstanceCountMap;
	}

	private void getModuleDownstreamInstances(int deviceId,
											  Map<Integer, Map<String, List<Integer>>> downstreamInstances) {
		if (this.getModuleInstanceListMap() == null) {
			return;
		}

		for (int childId: getDeviceById(deviceId).getChildrenIds()) {
			getModuleDownstreamInstances(childId,
										 downstreamInstances);

			for (String module: this.getModuleInstanceListMap().get(childId).keySet()) {
				List<Integer> childInstances = downstreamInstances.get(childId).get(module);
				if (!downstreamInstances.get(deviceId).containsKey(module)) {
					downstreamInstances.get(deviceId).put(module, new ArrayList<>());
				}

				for (Integer instance: childInstances) {
					if (!downstreamInstances.get(deviceId).get(module).contains(instance)) {
						downstreamInstances.get(deviceId).get(module).add(instance);
					}
				}
			}
		}

		if (this.getModuleInstanceListMap().containsKey(deviceId)) {
			for (String module: this.getModuleInstanceListMap().get(deviceId).keySet()) {
				List<Integer> deviceInstances = getModuleInstanceListMap().get(deviceId).get(module);
				if (!downstreamInstances.get(deviceId).containsKey(module)) {
					downstreamInstances.get(deviceId).put(module, new ArrayList<>());
				}

				for (Integer instance: deviceInstances) {
					if (!downstreamInstances.get(deviceId).get(module).contains(instance)) {
						downstreamInstances.get(deviceId).get(module).add(instance);
					}
				}
