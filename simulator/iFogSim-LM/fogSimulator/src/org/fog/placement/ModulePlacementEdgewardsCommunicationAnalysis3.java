...
public class ModulePlacementEdgewardsCommunicationAnalysis3 extends ModulePlacement{
    ...
    public ModulePlacementEdgewardsCommunicationAnalysis3(List<FogDevice> fogDevices, List<Sensor> sensors, List<Actuator> actuators,
                                                          Application application, ModuleMapping moduleMapping,
                                                          Map<Integer, Double> globalCPULoad, Map<String, Application> applications,
                                                          boolean debugMode){
...
//                         check if can place the new module in the device
                        double cpuNeededByEdeges = getApplication().getEdges().stream()
                                ...
                                .filter(appEdge -> appEdge.getDestination().equals(moduleName))
                                .map(appEdge -> ((UniformDistribution)appEdge.getTupleCpuLength()).getMeanInterTransmitTime())
                        ...
                        Map<String, Double> tempoExecutarTarefaModulo = calcularTempoExecucaoPorModuloe(deviceId, 1);
                        Map<String, Double> tempoExecutarTarefaModuloComReducao = calcularTempoExecucaoPorModuloe(deviceId, reduction);

                        double diferenca = 0;

                        for(Map.Entry<String, Double> module : tempoExecutarTarefaModulo.entrySet()){
                            diferenca += (
                                    tempoExecutarTarefaModuloComReducao.get(module.getKey()) - module.getValue()
                            );
                        }

                        boolean sobrecarregar = false;
                        boolean temEspaco = totalCpuLoad + currentCpuLoad <= (getFogDeviceById(deviceId).getHost().getTotalMips()*1.2);
...
//                        boolean melhorManter = melhorManter(deviceId, getApplication().getModuleByName(moduleName), timeToUp);
//                        if(timeToUp>diferenca && !device.getName().startsWith("m-") && !device.getName().startsWith("proxy")){
//                            sobrecarregar = true;
//                        }
...
//        getFogDeviceById(deviceId).getUplinkLatency()
    ...
    Map<String, Double> moduleFrequencyReceiver = new HashMap<>(){{
        put("concentration_calculator_1", 15.04/2);
