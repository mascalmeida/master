					...
					if(upsteamDeviceId==deviceId){
						...
						for(AppEdge edge : getApplication().getEdges()){		// take all incoming edges
							if(edge.getDestination().equals(moduleName)){
								...
								totalCpuLoad += rate*edge.getTupleCpuLength().getMeanInterTransmitTime();
							}
						}
						if(totalCpuLoad + getCurrentCpuLoad().get(deviceId) >
							...
							List<String> _placedOperators = shiftModuleNorth(moduleName, totalCpuLoad,
									deviceId, modulesToPlace);
