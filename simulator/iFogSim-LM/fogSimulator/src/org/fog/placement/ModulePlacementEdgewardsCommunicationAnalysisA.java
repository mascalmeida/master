...
public class ModulePlacementEdgewardsCommunicationAnalysisA extends ModulePlacement{
    ...
    public ModulePlacementEdgewardsCommunicationAnalysisA(List<FogDevice> fogDevices, List<Sensor> sensors, List<Actuator> actuators,
...
//        put("connector_1", 5.0);
    ...
    private double cpuNeeded(Application applications, String module){
            double cpuNeed = applications.getEdges().stream()
                    ...
                    .map(appEdge -> appEdge.getTupleCpuLength().getMeanInterTransmitTime() /
                            getFrequency(applications.getAppId(), module))
...
//						}
                        ...
                        totalCpuLoad = cpuNeeded(getApplication(), moduleName);
                        if (this.debugMode) {
...
//					}
    ...
    private List<String> shiftModuleWithLessCommunicationImpactNorth(String moduleName, Integer deviceId, List<String> modulesToPlace) {
        ...
        for(String module : modulesToShift){
            ...
            double moduleCpuLoad = cpuNeeded(getApplication(), module)*moduleCount;
            if (this.debugMode) {
            ...
            loadMap.put(module, loadMap.get(module)+cpuNeeded(getApplication(), module));
        }
        ...
        for(String module : modulesToShift) {
            if(moduleToNumInstances.get(module) == lowestInstanceNumber) {
                ...
                double moduleCpuLoad = cpuNeeded(getApplication(), module);
                totalCpuLoad += moduleCpuLoad;
            }
        }
...
//						totalCpuLoad += getCurrentDeviceModuleLoadMap().get(id).get(module);
                        ...
                        double moduleCpuLoad = cpuNeeded(getApplication(), module)*deviceModuleCount;
                        if (this.debugMode) {
