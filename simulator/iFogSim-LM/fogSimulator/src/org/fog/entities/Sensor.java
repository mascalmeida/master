...
public class Sensor extends SimEntity{
	...
	private int associatedAppInstance;
	...
	public Sensor(String name, int userId, String appId, int gatewayDeviceId, double latency, GeoLocation geoLocation,
		...
		this(name, userId, appId, gatewayDeviceId, latency, geoLocation,
			 transmitDistribution, cpuLength, nwLength,tupleType,
			 destModuleName, 0);
	}

	public Sensor(String name, int userId, String appId, int gatewayDeviceId, double latency, GeoLocation geoLocation,
				  Distribution transmitDistribution, int cpuLength, int nwLength, String tupleType, String destModuleName,
				  int associatedAppInstance) {
		super(name);
		this.setAppId(appId);
		...
		setLatency(latency);
		setAssociatedAppInstance(associatedAppInstance);
	}
	...
	public Sensor(String name, int userId, String appId, int gatewayDeviceId, double latency, GeoLocation geoLocation,
		...
		this(name, userId, appId, gatewayDeviceId, latency, geoLocation,
			 transmitDistribution,tupleType, 0);
	}

	public Sensor(String name, int userId, String appId, int gatewayDeviceId, double latency, GeoLocation geoLocation, 
			Distribution transmitDistribution, String tupleType, int associatedAppInstance) {
		super(name);
		this.setAppId(appId);
		this.gatewayDeviceId = gatewayDeviceId;
		this.geoLocation = geoLocation;
	...
	public Sensor(String name, String tupleType, int userId, String appId, Distribution transmitDistribution) {
		this(name, tupleType, userId, appId, transmitDistribution, 0);
	}
	...
	public void transmit(){
		...
		long cpuLength = (long) _edge.getTupleCpuLength().getNextValue();
		long nwLength = (long) _edge.getTupleNwLength();
		
		Tuple tuple = new Tuple(getAppId(), FogUtils.generateTupleId(), Tuple.UP, cpuLength, 1, nwLength, outputSize, 
				new UtilizationModelFull(), new UtilizationModelFull(), new UtilizationModelFull(), associatedAppInstance);
...
//		Logger.debug(getName(), "Sending tuple "+tuple.getCloudletId()+"to "+tuple.getDestModuleName()+" with delay="+delay);
	...
	public int getAssociatedAppInstance() {
		return associatedAppInstance;
	}

	public void setAssociatedAppInstance(int associatedAppInstance) {
		this.associatedAppInstance = associatedAppInstance;
	}
