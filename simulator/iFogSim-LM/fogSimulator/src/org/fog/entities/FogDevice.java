...
public class FogDevice extends PowerDatacenter {
	...
	protected Map<String, Map<String, List<Integer>>> moduleDownstreamInstanceList;

	protected boolean isLowLevel = false;
	...
	public FogDevice(
		...
		for (Host host : getCharacteristics().getHostList()) {
			...
			Log.printLine("Device:" +name + " Host: " + host.getId());
		}
		...
		setModuleDownstreamInstanceList(new HashMap<String, Map<String, List<Integer>>>());
	...
	public FogDevice(
		...
		System.out.println("Device:" +name + " Host: " + hostId);
	...
	private void updateModuleInstanceCount(SimEvent ev) {
		...
		List<Integer> downstreamInstances = config.getDownstreamInstanceList();
		if (!downstreamInstances.isEmpty()) {
			if(!moduleDownstreamInstanceList.containsKey(appId)) {
				moduleDownstreamInstanceList.put(appId, new HashMap<String, List<Integer>>());
			}
			moduleDownstreamInstanceList.get(appId).put(config.getModule().getName(), downstreamInstances);
		}
	}
	...
	private void sendPeriodicTuple(SimEvent ev) {
		...
		if (getModuleDownstreamInstanceList().containsKey(module.getAppId())
			&& getModuleDownstreamInstanceList().get(module.getAppId()).containsKey(srcModule)) {

			instanceList = getModuleDownstreamInstanceList().get(module.getAppId()).get(srcModule);
		}
		...
		for(int i = 0;i<((edge.getDirection()==Tuple.UP)?instanceCount:1);i++){
			int associatedInstance = 0;
			if (i < instanceList.size()) {
				associatedInstance = instanceList.get(i);
			}

			//System.out.println(applicationMap.get(module.getAppId()));
			Tuple tuple = applicationMap.get(module.getAppId()).createTuple(edge, getId(), associatedInstance);
				...
				while (vm.getCloudletScheduler().isFinishedCloudlets()) {
					...
					if (cl != null) {
						...
						List<Tuple> resultantTuples = application.getResultantTuples(tuple.getDestModuleName(), tuple,
																					 getId(), tuple.getAssociatedModuleInstance());
...
//        put("concentration_calculator_1", 15.04);
//        put("connector_1", 100.0);
	}};
	...
	private double cpuNeeded(String applicationName, String module){
		double cpuNeed = applicationMap.get(applicationName).getEdges().stream()
		...
		for(final Vm vm : getHost().getVmList()){
			if(vm.getCloudletScheduler().runningCloudlets() > 0 || ((AppModule)vm).getName().equals(incomingOperator)){
				getHost().getVmScheduler().allocatePesForVm(vm, new ArrayList<Double>(){
					...
					double cpu = cpuNeeded(appModule.getAppId(),appModule.getName());
					int count = moduleInstanceCount.get(appModule.getAppId()).get(appModule.getName());
				{add(
						(double) getHost().getTotalMips()
				);}});
			}else{
				getHost().getVmScheduler().allocatePesForVm(vm, new ArrayList<Double>(){
					protected static final long serialVersionUID = 1L;
...
//		System.out.println(getName() + " received tuple. Source: " + tuple.getSrcModuleName() + ", dest "
//							   + tuple.getDestModuleName() + ", instance: " + tuple.getAssociatedModuleInstance());

		if(appToModulesMap.containsKey(tuple.getAppId())){

			Boolean shouldConsiderIndividualInstances = getModuleDownstreamInstanceList().containsKey(tuple.getAppId())
					&& tuple.getAssociatedModuleInstance() > 0;
			List<Integer> destinationModuleInstances = new ArrayList<>();
			if (shouldConsiderIndividualInstances
					&& getModuleDownstreamInstanceList().get(tuple.getAppId()).containsKey(tuple.getDestModuleName())) {
				destinationModuleInstances =
						getModuleDownstreamInstanceList().get(tuple.getAppId()).get(tuple.getDestModuleName());
			}


//			if (shouldConsiderIndividualInstances) {
//				System.out.println(getName() + " has instances " + destinationModuleInstances + " of "
//									   + tuple.getDestModuleName());
//			} else if (appToModulesMap.get(tuple.getAppId()).contains(tuple.getDestModuleName())) {
//				System.out.println(getName() + " has instance of " + tuple.getDestModuleName());
//			} else {
//				System.out.println(getName() + " has no instance of " + tuple.getDestModuleName());
//			}

			if(appToModulesMap.get(tuple.getAppId()).contains(tuple.getDestModuleName())
					&& (!shouldConsiderIndividualInstances
					|| destinationModuleInstances.contains(tuple.getAssociatedModuleInstance()))){
				int vmId = -1;
				for(Vm vm : getHost().getVmList()){
	...
	protected void updateTimingsOnReceipt(Tuple tuple) {
		...
		if(app == null){
			System.out.println("impossivel atualziar updateTimingsOnReceipt");
			return;
		}
...
//		System.out.println(getName() + " executing (" + tuple.getSrcModuleName() + ", " + tuple.getDestModuleName() + ")");
...
//		System.out.println(getName() + " sending (" + tuple.getSrcModuleName() + ", " + tuple.getDestModuleName() + ") up.");
		if(parentId > 0){
			if(!isNorthLinkBusy()){
				sendUpFreeLink(tuple);
			}else{
...
//		System.out.println(getName() + " sending (" + tuple.getSrcModuleName() + ", " + tuple.getDestModuleName() + ") down.");
	...
	public Map<String, Map<String, List<Integer>>> getModuleDownstreamInstanceList() {
		return moduleDownstreamInstanceList;
	}

	public void setModuleDownstreamInstanceList(Map<String, Map<String, List<Integer>>> moduleDownstreamInstanceList) {
		this.moduleDownstreamInstanceList = moduleDownstreamInstanceList;
	}

	@Override
	public String toString() {
		return "["+this.getName()+" - Total CPU:"+this.getHost().getTotalMips()+"mips ]";
	}
}
