...
public class Application {
	...
	public void addAppModule(String moduleName, int ram) {
		addAppModule(moduleName, ram, 1000);
	}
	 ...
	 * @param edgeType
	 */
	public void addAppEdge(String source, String destination, Distribution tupleCpuLength,
	 ...
	 * @param edgeType
	 */
	public void addAppEdge(String source, String destination, double periodicity, Distribution tupleCpuLength,
			double tupleNwLength, String tupleType, int direction, int edgeType){
	 ...
	 * @return
	 */
	public List<Tuple> getResultantTuples(String moduleName, Tuple inputTuple, int sourceDeviceId, int associatedModuleInstance){
					...
					if(edge.getEdgeType() == AppEdge.ACTUATOR){
						//for(Integer actuatorId : module.getActuatorSubscriptions().get(edge.getTupleType())){
							Tuple tuple = new Tuple(appId, FogUtils.generateTupleId(), edge.getDirection(),  
									(long) (edge.getTupleCpuLength().getNextValue()),
									...
									inputTuple.getUtilizationModelBw(), associatedModuleInstance
									);
	 ...
	 * @return
	 */
	public Tuple createTuple(AppEdge edge, int sourceDeviceId, int associatedModuleInstance){
		...
		if(edge.getEdgeType() == AppEdge.ACTUATOR){
			for(Integer actuatorId : module.getActuatorSubscriptions().get(edge.getTupleType())){
				Tuple tuple = new Tuple(appId, FogUtils.generateTupleId(), edge.getDirection(),  
						...
						new UtilizationModelFull(), 
						new UtilizationModelFull(), associatedModuleInstance
						);
